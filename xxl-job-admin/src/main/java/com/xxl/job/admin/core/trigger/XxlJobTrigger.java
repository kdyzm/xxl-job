...
public class XxlJobTrigger {
    ...
    private static void processTrigger(XxlJobGroup group,
        ...
        triggerParam.setAppName(group.getAppname());
        ...
        if (CollUtil.isNotEmpty(webSocketServers)) {
            ...
            routeAddressResult = new ReturnT<WebSocketServer>(ReturnT.FAIL_CODE, "未发现有效的websocket连接");
        }
...
//        }
        ...
        triggerMsgSb.append("<br>").append(I18nUtil.getString("jobconf_trigger_exe_regaddress")).append("：").append(WebSocketServerPool.getInstance().getRegisterIps(group.getAppname())
        );
     ...
     * @return
     */
    public static ReturnT<String> runExecutor(TriggerParam triggerParam, WebSocketServer address) {
        ReturnT<String> runResult = null;
        try {
            ExecutorBiz executorBiz = XxlJobScheduler.getExecutorBiz(triggerParam.getAppName());
            runResult = executorBiz.run(address, triggerParam);
        } catch (Exception e) {
        ...
        runResultSB.append("<br>address：").append(address.getClientIp());
